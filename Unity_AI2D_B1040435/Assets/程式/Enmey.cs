...
public class Enmey : MonoBehaviour {
    ...
    {
        if (collision.gameObject.name == "鴨子")
        {
            collision.gameObject.GetComponent<duck>().Damge(damge);
        }
    }

    public void OnTriggerStay2D(Collider2D collision)
    {
        if (collision.name == "鴨子" && collision.transform.position.y < transform.position.y + 1)
        {
